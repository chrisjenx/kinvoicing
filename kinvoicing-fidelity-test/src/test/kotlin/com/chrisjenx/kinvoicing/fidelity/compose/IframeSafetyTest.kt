...
class IframeSafetyTest {
                ...
                listOf("href", "src").forEach { attr ->
                    ...
                    if (value.isNotEmpty()) {
                        assertTrue(
                            ...
                            "$name: <${el.tagName()}> $attr contains data:text/html URL"
                        )
                    }
                }
            }
                ...
                listOf("href", "src").forEach { attr ->
                    ...
                    if (value.isNotEmpty()) {
                        assertTrue(
                            ...
                            "$name: <${el.tagName()}> $attr contains vbscript: URL"
                        )
                    }
                }
            }
    ...
    @Test
    fun linksHaveRelNoopener() {
        forEachFixture { name, doc ->
            doc.select("a[target=_blank]").forEach { el ->
                val rel = el.attr("rel")
                assertTrue(
                    "noopener" in rel,
                    "$name: <a href=\"${el.attr("href")}\"> with target=\"_blank\" is missing rel=\"noopener\""
                )
            }
        }
    }
