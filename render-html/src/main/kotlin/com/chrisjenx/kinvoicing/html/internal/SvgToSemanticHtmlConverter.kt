...
import com.chrisjenx.kinvoicing.html.PdfHoverAnnotation
...
internal object SvgToSemanticHtmlConverter {
    ...
    fun convert(
        ...
        return buildString {
            ...
            for ((pageIndex, svg) in svgPages.withIndex()) {
                ...
                for (link in links) {
                    appendLine("<a href=\"${escapeAttr(sanitizeUrl(link.href))}\" target=\"_blank\" rel=\"noopener noreferrer\" class=\"link-overlay\" style=\"left:${fmtPt(link.x)};top:${fmtPt(link.y)};width:${fmtPt(link.width)};height:${fmtPt(link.height)}\"></a>")
                }
    ...
    private fun sanitizeUrl(url: String): String {
        ...
        require(scheme in SAFE_URL_SCHEMES) { "Unsafe URL scheme '$scheme' in href" }
        return url
    }
