...
public data class HoverStyles(
    ...
    init {
        val forbidden = setOf('{', '}', '<', '>', ';', '"', '\'')
        fun validate(name: String, value: String?) {
            if (value == null) return
            require(forbidden.none { it in value }) { "HoverStyles.$name contains forbidden character" }
            val lower = value.lowercase()
            require("url(" !in lower && "expression(" !in lower && "@import" !in lower) {
                "HoverStyles.$name contains forbidden CSS function"
            }
        }
        validate("backgroundColor", backgroundColor)
        validate("transform", transform)
        validate("boxShadow", boxShadow)
        validate("cursor", cursor)
        opacity?.let { require(it in 0f..1f) { "HoverStyles.opacity must be 0..1, got $it" } }
    }
}
