...
public class LineItemBuilder internal constructor(
    ...
    public fun sub(
        ...
        amount?.let { requireFinite(it, "sub-item amount") }
        qty?.toDouble()?.let { requireFinite(it, "sub-item quantity") }
        unitPrice?.let { requireFinite(it, "sub-item unitPrice") }
    ...
    internal fun build(): LineItem {
        explicitAmount?.let { requireFinite(it, "amount") }
        quantity?.let { requireFinite(it, "quantity") }
        unitPrice?.let { requireFinite(it, "unitPrice") }
...
internal fun buildAdjustmentValue(percent: Double?, fixed: Double?, label: String): AdjustmentValue = when {
    percent != null -> { requireFinite(percent, "$label percent"); AdjustmentValue.Percent(percent) }
    fixed != null -> { requireFinite(fixed, "$label fixed"); AdjustmentValue.Fixed(fixed) }
