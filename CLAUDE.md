...
- **CHANGELOG.md is reconciled at release time** (see `docs: reconcile CHANGELOG for v1.2.0 release` commit), not per-PR. The `[Unreleased]` section stays empty between releases; do not add entries as PRs land. GitHub Release notes are auto-generated by `gh release create --generate-notes` from PR titles, independent of CHANGELOG.
