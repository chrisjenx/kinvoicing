...
/**
 * Cross-platform image renderer that handles both [ImageSource.Bytes] (platform-specific decode)
 * and [DrawableImageSource] (native `painterResource`).
 *
 * When [width] and [height] are both specified, renders at that exact size.
 * Otherwise, uses sensible defaults (200x60 dp max) to match the HTML email logo constraint.
 */
@Composable
public fun InvoiceImageRenderer(
    ...
    when (source) {
        is DrawableImageSource -> {
            ...
            val decoded = remember(source) { decodeImageBytes(source.bytes) }
            Image(
